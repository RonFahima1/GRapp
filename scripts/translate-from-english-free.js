...
const translator = require('google-translator').translator;
...
const CONFIG = {
  ...
  rateLimit: {
    ...
    // Delay between batches (ms) - higher value for free service
    delayBetweenRequests: 1000,
    // Characters per batch - smaller for free service
    charsPerBatch: 500,
...
// Translation function using google-translator
async function translateText(text, sourceLang, targetLang) {
  ...
  try {
    ...
    // Use google-translator library (free)
    return new Promise((resolve, reject) => {
      translator.translate({
        ...
        if (result && result.translation) {
          ...
          reject(new Error('Failed to translate text'));
        }
      });
    });
  } catch (error) {
...
async function processLanguage(langCode) {
  ...
  const batchSize = Math.max(5, Math.floor(CONFIG.rateLimit.charsPerBatch / 30)); // Average 30 chars per string
  const batches = [];
...
if (showHelp) {
  log(`
🌐 Free Auto-Translation Script for i18n files
=============================================
...
Usage:
  node translate-from-english-free.js [options]
...
Examples:
  node translate-from-english-free.js             # Normal run
  node translate-from-english-free.js --dry-run   # Test without writing files
  node translate-from-english-free.js --force-all # Override all existing translations
  `, colors.bright);
  process.exit(0);
}
